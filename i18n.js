...
function detectLanguage(){ const pref=(localStorage.getItem('lang')||window.APP_CONFIG.DEFAULT_LANGUAGE||'auto'); if(pref!=='auto') return pref; const nav=(navigator.language||'').toLowerCase(); return nav.startsWith('ar')?'ar':'en'; }
...
function toggleLanguage(){ const lang=window.I18nState.lang==='ar'?'en':'ar'; applyLanguage(lang); if(typeof window.renderCharts==='function'){ window.renderCharts(); } }
window.addEventListener('DOMContentLoaded', ()=>{ applyLanguage(detectLanguage()); });