...
export const I18nState = { lang: 'ar' };
...
export function applyLanguage(lang){
  ...
  if (lt) lt.textContent = dict.lang_toggle || (lang==='ar'?'EN':'ع');
}

export function t(key){
  const d = I18N[I18nState.lang] || I18N.ar;
  return d[key] || key;
}

export function toggleLanguage(){
  const lang = I18nState.lang === 'ar' ? 'en' : 'ar';
  applyLanguage(lang);
  if (typeof window.renderCharts==='function') window.renderCharts();
  if (typeof window.rerenderAll==='function') window.rerenderAll();
}

// طبّق اللغة عند التحميل
window.addEventListener('DOMContentLoaded', ()=>{
  const cfg = window.CONFIG || {};
  if (cfg.DEFAULT_LANGUAGE && cfg.DEFAULT_LANGUAGE!=='auto') applyLanguage(cfg.DEFAULT_LANGUAGE);
  else applyLanguage(detectLanguage());
});
