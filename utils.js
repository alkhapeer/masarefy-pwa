...
export function getNumberLocale(){ const s=getSettingsRaw(); return s.numberLocale|| (window.CONFIG?.NUMBER_LOCALE || 'en-US'); }
export function getCurrencySymbol(){ const s=getSettingsRaw(); return s.currencySymbol|| (window.CONFIG?.CURRENCY_SYMBOL || 'ج.م'); }
...
export function toast(msg){ const t=document.createElement('div'); t.className='toasty'; t.textContent=msg; document.body.appendChild(t); setTimeout(()=>t.classList.add('show'),10); setTimeout(()=>{t.classList.remove('show'); setTimeout(()=>t.remove(),300);},2200); }